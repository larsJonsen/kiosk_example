defmodule HX712Server do
  ...
  defstruct [:addo, :adsk, :scale, :offset, :name, :pubsub]
  ...
  def read_load(name) do
    GenServer.call(name, :read_load, 5)
  end
  ...
  def calibrate(name) do
    GenServer.call(name, :calibrate, 100_000)
  end
  ...
  def init(%{
    ...
    with {:ok, %{addo: addo, adsk: adsk}} <- hx712_start(%{addo_pin: addo_pin, adsk_pin: adsk_pin}),
        {:ok, state} <- do_state(%{ addo: addo, adsk: adsk, scale: scale, 
                                  offset: offset, name: name, pubsub: pubsub }),
  ...
  defp do_state(%{ addo: addo, adsk: adsk, scale: scale, 
    offset: offset, name: name, pubsub: pubsub }) do 
    try do
       {:ok, %__MODULE__{
           addo: addo,
           adsk: adsk,
...
#    # Tiny delay (NOP equivalent) 
#    Process.sleep(1)
#    Circuits.GPIO.write(adsk, 0)
#    # IO.inspect(%{Time: Time.utc_now, Value: value})
#    {:ok, (bxor(value, 0x800000) - offset) / scale}
#  end
#
